 ...
 * page's top-of-page summary photo (never per-listing — a per-listing
 * version of this was tried and removed: with the listing's own exact
 * title as `query`, the epid-filtered search almost always found that
 * exact same listing back as its own best match, self-matching rather
 * than showing an independent photo). eBay's Catalog API (which would
 * resolve an epid directly to a product photo without needing a live
 * listing) requires a permission scope this app's key doesn't have
 * (confirmed live: 403 "Insufficient permissions"); filtering a normal
 * Browse API search by epid works with the same basic scope already used
 * elsewhere and returns the exact matching product — verified live
 * against the Luka Doncic Prizm card, whose epid it returned exactly.
 */
