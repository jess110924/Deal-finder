...
// Paging forward to satisfy `minProfitable` (see lib/playerSearch.ts's
...
// as check-watchlist/discover/auctions (see their route files).
...
export async function GET(request: NextRequest) {
  ...
  try {
    const result = await searchPlayerCards(query, category, {
      minPriceDollars,
      maxPriceDollars,
      sortBy,
